...
@admin_router.post("/", response_model=CertificationResponse, status_code=status.HTTP_201_CREATED)
def create_certification(certification_in: CertificationCreate, db: Session = Depends(get_db)):
...
def update_certification(certification_id: int, certification_in: CertificationUpdate, db: Session = Depends(get_db)):
    certification = CertificationService.get_or_404(db, certification_id)
    return CertificationService.update(db, certification, **certification_in.model_dump())
