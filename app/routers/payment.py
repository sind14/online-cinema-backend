...
def get_my_payments(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def create_payment(payment_data: PaymentCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
