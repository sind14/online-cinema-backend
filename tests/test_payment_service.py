...
def test_create_payment(
    ...
    payment = PaymentService.create_payment(
        db_session, active_user, PaymentCreate(order_id=order.id)
    )
...
def test_create_payment_raises_for_not_pending_order(
...
def test_create_payment_raises_for_nonexistent_order(db_session, active_user):
    with pytest.raises(HTTPException) as exc_info:
        PaymentService.create_payment(
            db_session, active_user, PaymentCreate(order_id=999)
        )
...
def test_get_payment_raises_for_other_user_payment(
    db_session, active_user, payment_factory, user_factory, order_factory
):
